...
# define SPRO_ENERGY_THD -150.0      /* energy thd for vad                    */
...
/* fea cmvn */
int fea_cmvn(spfstream_t *s);

/* vad according to energy */
int vad_energy(float *energy, float thd, int frame_num, bool *lbl);

int vad_to_trans(const bool *pValid, const int &frame_num, const char* szTrsName, const char* szWavName);

int vad_merge_short(bool *lbl, int lbl_num, int thd, bool type = false);
#endif /* _spro_h_ */
